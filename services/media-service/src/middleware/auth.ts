...
if (config.COGNITO_USER_POOL_ID && config.COGNITO_USER_POOL_ID !== 'us-east-1_mockpool') {
...
export async function verifyJWT(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    if ((config.NODE_ENV === 'development' || config.NODE_ENV === 'test') && (!config.COGNITO_USER_POOL_ID || config.COGNITO_USER_POOL_ID === 'us-east-1_mockpool')) {
